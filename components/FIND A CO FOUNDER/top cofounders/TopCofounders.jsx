...
const TopCofounders = () => {
  ...
  const nextSlide = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % (coFounders.length - 2));
  };

  const prevSlide = () => {
    setCurrentIndex(
      (prevIndex) =>
        (prevIndex - 1 + (coFounders.length - 2)) % (coFounders.length - 2)
    );
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex justify-center mt-4">
        {[...Array(coFounders.length - 2)].map((_, index) => (
          <button
            key={index}
